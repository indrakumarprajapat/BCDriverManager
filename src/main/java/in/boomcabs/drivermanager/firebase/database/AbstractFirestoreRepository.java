...
public abstract class AbstractFirestoreRepository<T> {
    ...
    public T getDriver(int driverId) {
        try {
            // asynchronously retrieve multiple documents
            ApiFuture<QuerySnapshot> future = collectionReference.whereEqualTo("driverId", driverId).get();
            // future.get() blocks on response
            List<QueryDocumentSnapshot> documents = future.get().getDocuments();
            if (documents.size() > 0) {
                DocumentSnapshot document = documents.get(0);
                return document.toObject(parameterizedType);
            }
